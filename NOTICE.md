# Notices and Acknowledgements

Houdi Agent is an independent open source project.

## Explicit Thanks

- OpenClaw.ai, for architectural inspiration around agentic operational workflows.
- Aurelio.ai (Semantic Router), for inspiration in semantic routing patterns.
- OpenAI, for foundational API capabilities used by this project.

## Trademark and Affiliation Notice

- This project is not officially affiliated with, endorsed by, or sponsored by OpenClaw.ai, Aurelio.ai, or OpenAI.
- This project is not officially affiliated with, endorsed by, or sponsored by GitHub, Inc.
- All product and company names may be trademarks of their respective owners.
