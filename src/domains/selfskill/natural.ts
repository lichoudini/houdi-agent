...
export function isLikelySelfSkillDraftLine(text: string, textNormalized: string): boolean {
  ...
  const likelyOtherDomain =
    /\b(envia|enviar|correo|gmail|email|mail|workspace|archivo|carpeta|noticias|web|internet|recordame|recordar|agenda|tarea|reboot|reinicia|actualiza)\b/.test(
      textNormalized,
    );
