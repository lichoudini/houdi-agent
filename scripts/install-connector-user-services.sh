...
APP_DIR="${CONNECTOR_APP_DIR:-./connector-app}"
APP_DIR="$(realpath "${APP_DIR}")"
SERVICE_DIR="${HOME}/.config/systemd/user"
APP_SERVICE="${CONNECTOR_APP_SERVICE:-${CONNECTOR_RETRIEVER_SERVICE:-houdi-connector-app.service}}"
TUNNEL_SERVICE="${CONNECTOR_TUNNEL_SERVICE:-${CONNECTOR_CLOUDFLARED_SERVICE:-houdi-connector-tunnel.service}}"
STACK_TARGET="houdi-connector-stack.target"
NPM_BIN="${NPM_BIN:-$(command -v npm || true)}"
TUNNEL_BIN="${TUNNEL_BIN:-$(command -v cloudflared || true)}"
TUNNEL_CONFIG="${CONNECTOR_TUNNEL_CONFIG:-${APP_DIR}/cloudflared/config.yml}"

log() { echo "[install-connector] $*"; }
fail() { echo "[install-connector][error] $*" >&2; exit 1; }

log "Instalador stack CONNECTOR (app + tunnel, systemd --user)"
...
[[ -d "${APP_DIR}" ]] || fail "No existe CONNECTOR_APP_DIR: ${APP_DIR}"
...
[Unit]
Description=Houdi CONNECTOR App
...
[Unit]
Description=Houdi CONNECTOR Tunnel
...
[Unit]
Description=Houdi CONNECTOR Stack
