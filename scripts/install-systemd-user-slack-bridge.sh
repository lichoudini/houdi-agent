...
SERVICE_FILE="${SERVICE_DIR}/houdi-slack-bridge.service"
...
log() { echo "[install-slack-user] $*"; }
fail() { echo "[install-slack-user][error] $*" >&2; exit 1; }
...
log "Instalador Slack bridge (systemd --user)"
log "Escribe ${SERVICE_FILE}"

cat > "${SERVICE_FILE}" <<SERVICE
[Unit]
Description=Houdi Slack Bridge
...
ExecStart=${NPM_BIN} run slack:bridge
...
systemctl --user enable --now houdi-slack-bridge.service

log "Servicio instalado."
echo "  1) Estado: systemctl --user status houdi-slack-bridge.service --no-pager"
echo "  2) Logs:   journalctl --user -u houdi-slack-bridge.service -f"
